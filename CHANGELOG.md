...
- **`pixeltamer doctor` now prints the skill version.** Header shows `pixeltamer: vX.Y.Z`, read straight from `package.json` with a sed-only parser — no `jq`/`python3` dependency, since doctor is exactly when those might be missing. Falls back to `vunknown` if `package.json` can't be found instead of erroring.
